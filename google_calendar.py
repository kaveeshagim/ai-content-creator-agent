...
def create_blog_event(title, scheduled_time):
    ...
    event = {
        ...
        "end": {
            "dateTime": (scheduled_time + datetime.timedelta(hours=1)).isoformat(),
...
def list_blog_events(max_results=100):
    """Return upcoming blog-related events from Google Calendar."""
    service = get_calendar_service()
    events_result = (
        service.events()
        .list(
